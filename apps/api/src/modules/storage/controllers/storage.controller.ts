...
export class StorageController {
        ...
        return implement(storageContract.object.list)
            ...
            .handler(async ({ input }) => {
                const result = await this.storageService.listObjects({
                    ...
                    recursive: input.query.recursive,
                    maxKeys: input.query.maxKeys,
                    continuationToken: input.query.continuationToken,
                });
        ...
        return implement(storageContract.object.delete)
            ...
            .handler(async ({ input }) => {
                try {
                    ...
                    return {
                        success: true,
                        message: `Object ${input.params.objectName} deleted successfully`,
                    };
                } catch (error) {
                    this.logger.error(`Error deleting object ${input.params.bucket}/${input.params.objectName}`, error);
                    return {
                        success: false,
    ...
    objectUpload() {
        return implement(storageContract.object.upload)
            ...
            .handler(async ({ input }) => {
                ...
                return {
                    name: input.body.objectName,
                    key: input.body.objectName,
                    size: result.size,
                    etag: result.etag,
    ...
    presignedDownload() {
        return implement(storageContract.presigned.download).handler(async ({ input }) => {
    ...
    presignedUpload() {
        return implement(storageContract.presigned.upload).handler(async ({ input }) => {
            const result = await this.storageService.handlePresignedUpload(input.token, input.file, input.contentType);
            return result;
        });
    }
        ...
        return implement(storageContract.object.head)
            ...
            .handler(async ({ input }) => {
                const result = await this.storageService.headObject({
                    ...
                    ifModifiedSince: input.headers?.["if-modified-since"] ? new Date(input.headers["if-modified-since"]) : undefined,
                    ifUnmodifiedSince: input.headers?.["if-unmodified-since"] ? new Date(input.headers["if-unmodified-since"]) : undefined,
                });
        ...
        return implement(storageContract.object.copy)
            ...
            .handler(async ({ input }) => {
                const result = await this.storageService.copyObjectWithMetadata({
                    sourceBucket: input.body.sourceBucket,
                    sourceKey: input.body.sourceKey,
                    destinationBucket: input.params.destinationBucket,
                    destinationKey: input.params.destinationKey,
                    metadataDirective: input.body.metadataDirective,
