    ...
    updateNode: (nodeId, updates) => {
        set((state) => {
            ...
            return {
                flow: {
                    ...state.flow,
                    nodes: state.flow.nodes.map((node) => (node.id === nodeId ? { ...node, ...updates } : node)),
                },
            };
        });
    },
        ...
        set((state) => {
            ...
            if (existingVar) {
                return {
                    flow: {
                        ...state.flow,
                        variables: state.flow.variables.map((v) => (v.name === name ? { ...v, value } : v)),
                    },
                    executionState: {
                        ...state.executionState,
                        variables: {
