...
describe('standard() + createRouterHooks() Integration', () => {
  ...
  describe('Hook Generation from standard() Contracts', () => {
    it('should generate query hooks for GET operations', () => {
      ...
      const listHook = hooks.useList as any;
      listHook({ limit: 10 });
    ...
    it('should generate mutation hooks for non-GET operations', () => {
      ...
      // Verify hooks can be called
      const createHook = hooks.useCreate as any;
      createHook();
    ...
    it('should generate all CRUD hooks from standard() operations', () => {
      ...
      const mockRouter = {
        list: {
          ...ops.list().build(),
          queryKey: ['testEntity.list'],
          queryOptions: () => ({ queryKey: ['testEntity.list'], queryFn: () => [] }),
        },
        read: {
          ...ops.read().build(),
          queryKey: ['testEntity.read'],
          queryOptions: () => ({ queryKey: ['testEntity.read'], queryFn: () => null }),
        },
        create: {
          ...ops.create().build(),
    ...
    it('should support typed invalidation configuration', () => {
      ...
      const mockRouter = {
        list: {
          ...ops.list().build(),
          queryKey: ['testEntity.list'],
          queryOptions: () => ({ queryKey: ['testEntity.list'] }),
        },
        read: {
          ...ops.read().build(),
          queryKey: ['testEntity.read'],
          queryOptions: () => ({ queryKey: ['testEntity.read'] }),
        },
        create: {
          ...ops.create().build(),
          mutationOptions: (opts: any) => ({ mutationFn: opts.input }),
  ...
  describe('Type Safety', () => {
    ...
    it('should properly type hook return values', () => {
      ...
      const listResult = (hooks.useList as any)();
      const createResult = (hooks.useCreate as any)();
  ...
  describe('Real-world Usage Pattern', () => {
    it('should work exactly like the user contracts in production', () => {
      ...
      expectTypeOf<z.infer<typeof userSchema>>().toMatchObjectType<{
        id: string;
        name: string;
        email: string;
